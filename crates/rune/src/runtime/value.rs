...
use crate::runtime::{
    AccessError, AccessErrorKind, AnyObj, AnyObjError, AnyRef, AnyRefDrop, BorrowMut, BorrowRef,
    Bytes, ConstValue, ControlFlow, DynGuardedArgs, EnvProtocolCaller, Format, Formatter,
    FromValue, Function, Future, Generator, GeneratorState, IntoOutput, Iterator, MaybeTypeOf, Mut,
    Object, OwnedTuple, Protocol, ProtocolCaller, Range, RangeFrom, RangeFull, RangeInclusive,
    RangeTo, RangeToInclusive, RawAnyRefGuard, RawRefGuard, Ref, RuntimeError, Shared, Snapshot,
...
impl Value {
        ...
        match self.into_repr()? {
            ...
            ValueRepr::Mutable(value) => {
                ...
                let value = match result {
                    ...
                    Err((error, _)) => {
                        return Err(RuntimeError::from(AccessError::from(error)));
                    }
                };
            ...
            ValueRepr::Ref(value) => {
                let (ptr, guard) = value.downcast_ref_ptr::<T>()?;
        ...
        match self.into_repr()? {
            ...
            ValueRepr::Mutable(value) => {
                ...
                let value = match result {
                    ...
                    Err((error, _)) => return Err(RuntimeError::from(AccessError::from(error))),
                };
            ...
            ValueRepr::Ref(value) => {
                let (ptr, guard) = value.downcast_mut_ptr::<T>()?;
        ...
        T: Any,
    {
        match self.value_ref()? {
            ...
            ValueRef::Ref(value) => Ok(value.borrow_ref()?),
        }
    }
        ...
        T: Any,
    {
        match self.value_ref()? {
            ...
            ValueRef::Ref(value) => Ok(value.borrow_mut()?),
        }
    }
